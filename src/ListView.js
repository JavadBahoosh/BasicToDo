...
class ListView extends Component {
    ...
    render() {
        ...
        if (this.state.dataList.length) {
            listView = (<SortableListView ref="listView" style={{flex: 1}}
                                          data={this.state.dataList} order={dataListOrder}
                                          onRowMoved={e => moveOrderItem(this, e.from, e.to)}
                                          renderRow={
                                              (dataItem, section, index) =>
                                                  <ListViewItem data={dataItem} dataIndex={index}
